<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px","background":"#ffffff"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","order":"2"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">发放日期</label>
						<el-date-picker v-model="searchForm.fafangriqistart" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="发放日期起始"></el-date-picker>
						至
						<el-date-picker v-model="searchForm.fafangriqiend" type="date" value-format="yyyy-MM-dd" format="yyyy-MM-dd" placeholder="发放日期结束"></el-date-picker>
					</div>
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yuangonggongzi','每月工资支出')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						每月工资支出
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #fff","padding":"0","color":"#000","background":"none","borderWidth":"0","width":"100%","fontSize":"14px"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true'  
						prop="kouchuwuxianyijin"
						label="扣除五险一金">
						<template slot-scope="scope">
							{{scope.row.kouchuwuxianyijin}}
						</template>
					</el-table-column>
					<el-table-column :resizable='true' :sortable='true'  
						prop="koushui"
		...
		<el-dialog
			...
			width="800">
			<div id="zuizhonggongziChart1" style="width:100%;height:600px;"></div>
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var zuizhonggongziChart1 = echarts.init(document.getElementById("zuizhonggongziChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							...
							// 使用刚指定的配置项和数据显示图表。
							zuizhonggongziChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								zuizhonggongziChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			getDataList() {
				...
				let params = {
 				...
 				if(this.searchForm.fafangriqistart!='' && this.searchForm.fafangriqistart!=undefined ){
					params['fafangriqistart'] = this.searchForm.fafangriqistart
				}
				if(this.searchForm.fafangriqiend!='' && this.searchForm.fafangriqiend!=undefined){
					params['fafangriqiend'] = this.searchForm.fafangriqiend
				}

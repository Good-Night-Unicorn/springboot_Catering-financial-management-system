<template>
	<div class="main-content" :style='{"width":"100%","padding":"20px 30px","fontSize":"15px","background":"#ffffff"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #fff","margin":"0 0px 0px","flexWrap":"wrap","background":"none","display":"flex","width":"100%","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"padding":"10px","alignItems":"center","flexWrap":"wrap","background":"none","display":"flex","order":"2"}' >
					<div :style='{"alignItems":"center","margin":"0 10px 0 0","display":"flex"}' class="select" label="考勤类型" prop="kaoqinleixing">
						<label :style='{"margin":"0 10px 0 0","whiteSpace":"nowrap","color":"#666","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">考勤类型</label>
						<el-select clearable v-model="searchForm.kaoqinleixing" placeholder="请选择考勤类型" >
				...
				<el-row class="actions" :style='{"padding":"10px","margin":"0px 0","flexWrap":"wrap","background":"none","display":"flex"}'>
					...
					<el-button class="btn18" v-if="isAuth('yuangongkaoqin','考勤统计')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"14px","color":"#fff","display":"none","height":"34px"}'></span>
						考勤统计
					</el-button>
				</el-row>
		...
		<el-dialog
			...
			width="800">
			<div id="kaoqinleixingChart1" style="width:100%;height:600px;"></div>
	...
	export default {
		...
		methods: {
			...
			chartDialog1() {
				...
				this.$nextTick(()=>{
					var kaoqinleixingChart1 = echarts.init(document.getElementById("kaoqinleixingChart1"),'macarons');
					this.$http({
						...
						if (data && data.code === 0) {
							let res = data.data;
// typeSections [com.jlwl.entity.TypeSection@4fa03c7f, com.jlwl.entity.TypeSection@4c0ef00b, com.jlwl.entity.TypeSection@464a1878, com.jlwl.entity.TypeSection@6e57172, com.jlwl.entity.TypeSection@24ff514f]
							let xAxis = [];
							let yAxis1 = [];
							let yAxis2 = [];
							let yAxis3 = [];
							...
							// 使用刚指定的配置项和数据显示图表。
							kaoqinleixingChart1.setOption(option);
							  //根据窗口的大小变动图表
							window.onresize = function() {
								kaoqinleixingChart1.resize();
							};
						}else{
							this.$message({
								message: data.msg,
			...
			init () {
				this.kaoqinleixingOptions = "正常,早退,迟到,补签,请假".split(',')
			},
			search() {
				this.pageIndex = 1;
				this.getDataList();

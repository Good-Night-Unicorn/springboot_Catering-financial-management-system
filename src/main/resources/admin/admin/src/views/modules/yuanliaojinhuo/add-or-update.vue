		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="进货单价" prop="jinhuodanjia" >
					<el-input v-model="ruleForm.jinhuodanjia" placeholder="进货单价" clearable  :readonly="ro.jinhuodanjia"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="进货单价" prop="jinhuodanjia" >
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `yuanliaojinhuo/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.yuanliaojinhuoCrossAddOrUpdateFlag = false;
											this.parent.search();

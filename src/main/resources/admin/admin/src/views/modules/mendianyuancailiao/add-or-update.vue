	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="input" v-if="type!='info'"  label="原料类型" prop="yuanliaoleixing" >
					<el-input v-model="ruleForm.yuanliaoleixing" placeholder="原料类型" clearable  :readonly="ro.yuanliaoleixing"></el-input>
				</el-form-item>
				<el-form-item v-else class="input" label="原料类型" prop="yuanliaoleixing" >
					<el-input v-model="ruleForm.yuanliaoleixing" placeholder="原料类型" readonly></el-input>
				</el-form-item>
			...
			<el-form-item class="textarea" v-if="type!='info'" label="原料详情" prop="yuanliaoxiangqing" >
				<el-input
			...
			<el-form-item v-else-if="ruleForm.yuanliaoxiangqing" label="原料详情" prop="yuanliaoxiangqing" >
				<span class="text">{{ruleForm.yuanliaoxiangqing}}</span>
			</el-form-item>
...
<script>
	...
	export default {
		...
		methods: {
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='yuanliaoleixing'){
							this.ruleForm.yuanliaoleixing = obj[o];
							this.ro.yuanliaoleixing = true;
							continue;
						}
						if(o=='guige'){
			...
			info(id) {
				this.$http({
					url: `mendianyuancailiao/info/${id}`,
					method: "get"
				}).then(({ data }) => {
					if (data && data.code === 0) {
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `mendianyuancailiao/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.mendianyuancailiaoCrossAddOrUpdateFlag = false;
											this.parent.search();

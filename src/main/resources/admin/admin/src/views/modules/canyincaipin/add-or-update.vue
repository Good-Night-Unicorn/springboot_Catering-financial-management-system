	...
	<div class="addEdit-block">
		...
		>
			<template >
				...
				<el-form-item class="select" v-if="type!='info'"  label="时令" prop="shiling" >
					<el-select :disabled="ro.shiling" v-model="ruleForm.shiling" placeholder="请选择时令" >
						<el-option
							v-for="(item,index) in shilingOptions"
							v-bind:key="index"
							:label="item"
							:value="item">
						</el-option>
				...
				<el-form-item v-else class="input" label="时令" prop="shiling" >
					<el-input v-model="ruleForm.shiling"
						placeholder="时令" readonly></el-input>
				</el-form-item>
			...
			init(id,type) {
				...
				if(this.type=='info'||this.type=='else'||this.type=='msg'){
					...
					for (var o in obj){
						...
						if(o=='shiling'){
							this.ruleForm.shiling = obj[o];
							this.ro.shiling = true;
							continue;
						}
						if(o=='caipinfenlei'){
			...
			info(id) {
				this.$http({
					...
					if (data && data.code === 0) {
						...
						this.ruleForm.jianjie = this.ruleForm.jianjie.replace(reg,'../../../springbootui41jy6y/upload');
					} else {
						this.$message.error(data.msg);
					}
				});
			...
			async onSubmit() {
					...
					await this.$refs["ruleForm"].validate(async valid => {
						if (valid) {
							...
							await this.$http({
								url: `canyincaipin/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(async ({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.canyincaipinCrossAddOrUpdateFlag = false;
											this.parent.search();
